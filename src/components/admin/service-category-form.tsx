...
export function ServiceCategoryForm({ action, initial, compact }: Props) {
  ...
  return (
    <form onSubmit={handleSubmit} className="grid gap-3 sm:grid-cols-[1fr_1.5fr_auto]">
      <div>
        <Label htmlFor="srv-cat-name">Nombre</Label>
        <Input id="srv-cat-name" name="name" required placeholder="Ej: Cerrajería" defaultValue={initial?.name} />
      </div>
      <div>
        <Label htmlFor="srv-cat-image">URL de imagen (opcional)</Label>
        <Input
          id="srv-cat-image"
          name="imageUrl"
          placeholder="https://…"
